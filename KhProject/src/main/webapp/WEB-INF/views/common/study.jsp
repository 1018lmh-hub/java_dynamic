...
<body>
	...
	<h2>AJAX!(진짜 중요함 뻥 아님)</h2>
	
	<pre>
		AJAX == Asynchronous JavaScript And XML
		
		"페이지를 새로고침하지 않고 서버와 데이터를 주고받을 수 있는 기술"\
		"비동기 통신 기술"
		...
		3. 브라우저는 응답받는 HTML데이터를 처음부터 끝까지 렌더링 -> 전체페이지를 다시 로딩 -> 페이지가 한번 깜빡함(SSR)
		
		비동기방식:
		1. 사용자가 요청을 보냄
		2. JavaScript기술을 이용해서 데이터만 서버로 띡 전송
		3. 서버는 JSON/XML데이터만 응답
		4. JavaScript를 이용해서 필요한 부분만 갱신 -> 부드럽고, 빠름(CSR)
	...
	<pre>
		...
		<reply>
			...
			<contnet>태준이형 600화 축하하고 수고했으면 개추 ㅋㅋㅋ</contnet>
		</reply>
		(구세대 방식)
		...
		사람이 읽기 쉽고, 기계가 파싱하기 쉬운 데이터 교환형식 텍스트 기반이라 아주 가볍다.
		
		* 진짜로 자바스크립트 객체 X / 자바스크립트 객체 모양으로 문자열을 만든거 (객체 표현식)
		...
		{
			name : "김예찬",										//키에 따옴표 안적어도 됨
			id : 'kyc0****',(이거 컴마도 없어도 되나)					// 작은 따옴표 사용가능
			content : '태준이형 600화 축하하고 수고했으면 개추 ㅋㅋㅋ' , 	// 마지막 속성에 컴마 가능
		}
	...
	<pre>
		...
		2. JQuery를 사용해서 ajax 메소드를 호출하는 방법			+
		3. fetch API를 활용해서 fetch 호출하는 방법				+
	...
	<div class="form-group">
		<div class="form-control">
			<button class="btn btn-sm btn-secondery" onclick="fn1()">요청보내기</button>
		</div>
	</div>
